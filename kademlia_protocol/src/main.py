...
async def main():
    ...
    if args.bootstrap:
        bootstrap_node = node.Node("0x000000000000000000".encode, "127.0.0.1", 8000)
        ...
        new_node = node.Node(hashlib.sha1(f"{args.ip}{args.port}".encode()).hexdigest(), args.ip, args.port)
        transport, protocol = await loop.create_datagram_endpoint(
